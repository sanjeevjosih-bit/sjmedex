...
const { createCashfreeOrder, verifyWebhookSignature } = require('./cashfreeService');
...
app.post('/api/orders', authMiddleware, async (req, res) => {
  ...
  const { items, payment_method } = req.body; // payment_method: 'cod' or 'online'
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO orders (order_number, pharmacy_id, items, subtotal, delivery_charge, total, status, payment_method, payment_status)
       VALUES ($1,$2,$3,$4,$5,$6,'pending',$7,$8) RETURNING *`,
      [orderNumber, req.pharmacy.id, JSON.stringify(orderItems), subtotal, deliveryCharge, total, method, method === 'online' ? 'pending' : 'not_required']
    );

    const order = result.rows[0];

    // If online payment chosen, create a Cashfree order and return the payment session
    if (method === 'online') {
      const pharmacyResult = await pool.query('SELECT * FROM pharmacies WHERE id = $1', [req.pharmacy.id]);
      ...
      try {
        const cfOrder = await createCashfreeOrder({
          orderId: orderNumber,
          amount: total,
          customerName: pharmacy.pharmacy_name,
          customerPhone: pharmacy.mobile,
          customerEmail: pharmacy.email,
          returnUrl: `https://sjmedex.in/payment-status?order_id=${orderNumber}`,
        });

        await pool.query('UPDATE orders SET cf_order_id = $1 WHERE id = $2', [cfOrder.cf_order_id || orderNumber, order.id]);
        ...
        console.error('Cashfree order creation failed:', cfErr.response?.data || cfErr.message);
        // Order is still saved as COD fallback so the pharmacy isn't stuck
        await pool.query('UPDATE orders SET payment_method = $1, payment_status = $2 WHERE id = $3', ['cod', 'not_required', order.id]);
        return res.json({ message: 'Order placed (payment gateway unavailable, defaulted to Cash on Delivery)', order });
      }
    }
...
// Cashfree webhook — confirms payment success/failure
app.post('/api/payments/webhook', async (req, res) => {
  try {
    ...
    const paymentStatus = payload.data?.payment?.payment_status; // SUCCESS, FAILED, etc.
    const cfPaymentId = payload.data?.payment?.cf_payment_id;

    if (orderId) {
      const newStatus = paymentStatus === 'SUCCESS' ? 'paid' : 'failed';
      await pool.query(
        'UPDATE orders SET payment_status = $1, cf_payment_id = $2 WHERE order_number = $3',
        [newStatus, cfPaymentId, orderId]
      );
      console.log(`Payment webhook: order ${orderId} → ${newStatus}`);
    }
...
// Check payment status for an order (used by frontend after redirect back)
app.get('/api/orders/:orderNumber/payment-status', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT order_number, payment_method, payment_status, status FROM orders WHERE order_number = $1 AND pharmacy_id = $2',
      [req.params.orderNumber, req.pharmacy.id]
    );
